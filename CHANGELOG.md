...
### Added
- Generated headers now embed a comment with the invocation switches, e.g.:
  `/* Generated by raw2header V3.02.0 with: -a -m */`

### Changed
- Conflict detection added for mutually exclusive flags:
  - `-m`/`--mono` and `-s`/`--stereo` now produce an error instead of silently picking the last one
  - `-16` and `-b16` now produce an error instead of silently picking the last one
